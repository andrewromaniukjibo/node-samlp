 ...
 * - getUserFromRequest(req) a function that given a request returns the user. By default request.user
 * - issuer string
...
module.exports.auth = function(options) {
  ...
  opts.getUserFromRequest = opts.getUserFromRequest || function(request){ return request.user; };
  ...
  function execute (postUrl, audience, request, reply, next) {
    var user = opts.getUserFromRequest(request);
    ...
    getSamlResponse(opts, user, function (err, SAMLResponse) {
      ...
      if (opts.responseHandler) {
        opts.responseHandler(response, opts, request, reply, next);
      } else {
        reply(templates.form({
          callback:        postUrl,
          RelayState:      opts.RelayState || (request.query || {}).RelayState || (request.body || {}).RelayState || '',
  ...
  return function (request, reply, next) {
    getSamlRequest((request.query || {}).SAMLRequest || (request.body || {}).SAMLRequest, function(err, samlRequestDom) {
      ...
      opts.getPostURL(audience, samlRequestDom, request, function (err, postUrl) {
        if (err) { return reply(err).code(500); }
        if (!postUrl) { return reply().code(401); }

        execute(postUrl, audience, request, reply, next);
      });
    });
  };
};

module.exports.parseRequest = function(request, callback) {
  var samlRequest = (request.query || {}).SAMLRequest || (request.body || {}).SAMLRequest;
  if (!samlRequest)
    return callback();
...
module.exports.logout = function (options) {
  function execute (postUrl, logoutRequestData, request, reply, next) {
    ...
    getLogoutResponse(options, function (err, logoutResponse) {
      ...
      reply(templates.form({
        callback:     postUrl,
        RelayState:   options.RelayState || (request.query || {}).RelayState || (request.body || {}).RelayState || '',
  ...
  return function (request, reply, next) {
    module.exports.parseLogoutRequest(request, function (err, logoutRequestData) {
      if (err) { return reply(err).code(500); }
      if (!logoutRequestData) { return reply('missing SAMLRequest').code(400); }
      ...
      options.getPostURL(logoutRequestData, request, function (err, postUrl) {
        if (err) { return rreply(err).code(500); }
        if (!postUrl) { return reply().code(401); }
        execute(postUrl, logoutRequestData, request, reply, next);
      });
    });
  };
};

module.exports.parseLogoutRequest = function (request, callback) {
  var samlRequest = (request.query || {}).SAMLRequest || (request.body || {}).SAMLRequest;
  if (!samlRequest) return callback();
...
module.exports.sendError = function (options) {
  ...
  function renderResponse(reply, postUrl) {
  ...
  return function (request, reply, next) {
    options.getPostURL(request, function (err, postUrl) {
      if (err) return next(err);
      if (!postUrl) return next(new Error('postUrl is required'));
      renderResponse(reply, postUrl);
    });
  };
};
